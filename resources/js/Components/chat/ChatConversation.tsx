...
import { Message, Todo, AutonomousPhase } from '@/types';
...
function AutonomousBanner({ phase, iteration, maxIterations }: { phase: AutonomousPhase; iteration: number; maxIterations: number }) {
    const phaseLabels: Record<string, string> = {
        working: 'Working autonomously',
        qa: 'QA Review',
        completed: 'Completed',
        failed: 'Failed',
    };
    const progress = maxIterations > 0 ? Math.min((iteration / maxIterations) * 100, 100) : 0;
    const isActive = phase === 'working' || phase === 'qa';

    return (
        <div className="mx-auto max-w-3xl mb-4">
            <div className={cn(
                'flex items-center gap-3 px-4 py-2.5 rounded-md border',
                phase === 'completed' ? 'bg-success/5 border-success/20' :
                phase === 'failed' ? 'bg-error/5 border-error/20' :
                'bg-bg-muted border-border'
            )}>
                {isActive && (
                    <span className="w-3.5 h-3.5 border-2 border-fg-muted border-t-fg rounded-full animate-spin flex-shrink-0" />
                )}
                {phase === 'completed' && <CheckCircleIcon className="w-4 h-4 text-success flex-shrink-0" />}
                <span className="text-xs font-medium text-fg flex-1">
                    {phaseLabels[phase] || phase}
                </span>
                <span className="text-[11px] text-fg-muted">{iteration}/{maxIterations}</span>
                <div className="w-16 h-1.5 bg-bg-accent rounded-full overflow-hidden">
                    <div
                        className={cn(
...
export function ChatConversation({ todo, messages, onNewMessage, className }: ChatConversationProps) {
    ...
    const {
        ...
        autonomousPhase, autonomousIteration, autonomousMaxIterations,
    ...
    return (
        <div className={cn('h-full flex flex-col bg-bg', className)}>
            ...
            {/* Autonomous banner */}
            {(todo.is_autonomous && autonomousPhase) && (
                <AutonomousBanner
                    phase={autonomousPhase}
                    iteration={autonomousIteration}
                    maxIterations={autonomousMaxIterations || todo.autonomous_max_iterations}
                />
            )}

            {/* Completion banner */}
            {showCompletionBanner && !isStreaming && !autonomousPhase && (
