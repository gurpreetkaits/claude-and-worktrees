...
export function TodoCreateForm({ worktree, models = defaultModels, defaultModel = 'sonnet', defaultContext = '', onCancel }: TodoCreateFormProps) {
    ...
    return (
        <div className="bg-bg-primary border border-border rounded-xl p-6 shadow-lg">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                    <SparklesIcon className="w-5 h-5 text-brand" />
                    <h2 className="text-lg font-semibold text-text-high">New Task</h2>
                </div>
                <button
                    type="button"
                    onClick={onCancel}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                {/* Title */}
                <div>
                    <label htmlFor="title" className="block text-sm font-medium text-text-normal mb-1">
                ...
                <div>
                    <label htmlFor="context" className="block text-sm font-medium text-text-normal mb-1">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {(Object.keys(models) as ClaudeModel[]).map((modelKey) => (
                            <button
                                ...
                                className={`p-3 text-left transition-all ${
                                    model === modelKey
                                        ...
                                        : 'bg-bg-panel border-2 border-transparent hover:border-brand/30'
                                }`}
                            ...
                            >
                                ...
                                <div className="text-xs text-text-low mt-0.5">
                ...
                <button
                    ...
                    className="flex items-center gap-1 text-sm text-text-low hover:text-text-normal transition-colors"
                >
                    <ChevronRightIcon
                ...
                {showAdvanced && (
                    <div className="space-y-4 p-4 bg-bg-secondary rounded-lg border border-border">
                        {/* Pre-Command */}
                        <div>
                            <label htmlFor="preCommand" className="block text-sm font-medium text-text-normal mb-1">
                                Pre-Command
                            </label>
                            <input
                                id="preCommand"
                        ...
                        {/* Post-Command */}
                        <div>
                            <label htmlFor="postCommand" className="block text-sm font-medium text-text-normal mb-1">
                                Post-Command
                            </label>
                            <input
                                id="postCommand"
                        ...
                        {/* Message Prefix */}
                        <div>
                            <label htmlFor="messagePrefix" className="block text-sm font-medium text-text-normal mb-1">
                                Message Prefix
                            </label>
                            <textarea
                                id="messagePrefix"
                        ...
                        {/* Message Suffix */}
                        <div>
                            <label htmlFor="messageSuffix" className="block text-sm font-medium text-text-normal mb-1">
                                Message Suffix
                            </label>
                            <textarea
                                id="messageSuffix"
